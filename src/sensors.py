...
class CameraManager(object):
    def __init__(self, parent_actor, hud, gamma_correction):
        ...
        self.sensors = [
            ...
            ['sensor.lidar.ray_cast', None, 'Lidar (Ray-Cast)', {'range': '80'}],
        ]
        ...
        for item in self.sensors:
            ...
            if item[0].startswith('sensor.camera'):
                bp.set_attribute('image_size_x', str(self.hud.width))
                bp.set_attribute('image_size_y', str(self.hud.height))
    ...
    def _parse_image(weak_self, image):
        ...
        if self.sensors[self.index][0].startswith('sensor.lidar'):
            ...
            lidar_data *= min(self.hud.width, self.hud.height) / (2.0 * self.lidar_range)
            lidar_data += (0.5 * self.hud.width, 0.5 * self.hud.height)
            lidar_data = np.fabs(lidar_data)
            lidar_data = lidar_data.astype(np.int32)
            lidar_data = np.reshape(lidar_data, (-1, 2))
            lidar_img_size = (self.hud.width, self.hud.height, 3)
